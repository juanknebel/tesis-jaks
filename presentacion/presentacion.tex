...
\begin{itemize}
	...
	\item Obtiene varias listas de discos:
		\begin{itemize}
			\item Por país.
			\item Por período.
		\end{itemize}
	\item Debe procesar manualmente todos los discos, cuidando respetar sus requerimientos al definir los discos a comprar.
\end{itemize}
}
...
\section{Recuperación de la información y recuperación  compuesta} 
\frame{\frametitle{Recuperación de la Información y Recuperación Compuesta}
\begin{itemize}
...
\begin{itemize}
	\item \textbf{Compatibilidad:} Los elementos dentro de un paquete deben ser similares. El grado de similitud de los elementos que forman un paquete definen su calidad.
	\textcolor{cyan}{Año de lanzamiento del disco}.
	\pause
	\item \textbf{Complementariedad:} Todos los paquetes cumplen que, para un mismo atributo especificado los elementos dentro del mismo paquete contienen no repiten tal propiedad.
	\textcolor{cyan}{País de origen del disco}.
	\pause
	\item \textbf{Validez:} El costo total de los elementos del paquete no puede superar el presupuesto.
	\textcolor{cyan}{Valor total de todos los discos}.
	\pause
	\item \textbf{Diversidad:} Los paquetes entre sí deben ser diversos.
	\textcolor{cyan}{Los años representados en los paquetes no deben solaparse}.
\end{itemize}
}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\end{itemize}
		\item BOBO-c (bundles one-by-one): Se generan $c*k$ paquetes seleccionando un ítem como pivote y en cada iteración de manera golosa construye un paquetes a su alrededor.
	\end{itemize}
	\pause
...
\begin{itemize}
	...
	\item SS:
	\begin{itemize}
		\item La componente inter de la función objetivo crece linealmente al agregarse un nuevo paquete a la solución. En contraste, la intra lo hace en forma cuadrática.
...
% Proporciona soluciones más diversas:
% \begin{itemize}
...
\tiny
\begin{itemize}
	\item Elige el paquete $b_{r}$ con menor valor inter de la solución actual $S$ para ser reemplazado:
	$$b_{r} = \argmin_{b_{j} \in \overline{S}} \sum_{\substack{b_{j} \in \overline{S}\\ b_{j} \neq b_{i}}} (1 - \max_{\substack{u \in b_{i}\\ v \in b_{j}}}s(u,v))$$
	\item Identifica el paquete $b_{c}$ con menor valor inter en $S \setminus \{b_{r}\}$.
	\item Arma una lista de paquetes candidatos con aquellos paquetes que quedaron fuera de la selección y que maximicen el valor inter hacia $b_{c}$:
	$$C = k-\argmax_{b_{j} \in B \setminus \overline{S}}(1 - \max_{\substack{u \in b_{i}\\ v \in b_{c}}}s(u,v))$$
	\item Selecciona el ítem $b_{s}$ de la lista de candidatos que maximice la suma total de los valores inter de la solución $S \setminus \{b_{r}\} \cup \{b_{s}\}$.
	\item Genera la nueva solución reemplazado $b_{r}$ por $b_{s}$.
	\item Evalúa el criterio de aspiración para los elementos tabú.
\end{itemize}
\normalsize
}
...
\begin{itemize}
	...
	\item Tres acciones posibles:
	\begin{enumerate}
		\tiny
		\item Intenta agregar un ítem $r$ de $C$ al paquete si es posible para aprovechar el presupuesto.
		\item Si no puede agregar ningún ítem selecciona el ítem $r$ de $C$ que no es tabú con mayor valor intra del paquete $b \setminus \{i\} \cup \{r\}$.
		\item Evalúa la solución reemplazando $b$ por $b \setminus \{i\}$.
	\end{enumerate}
	\item Elige la opción que mejora la función objetivo o que menos la empeora.
...
Una nueva propuesta que aborda el problema de una forma diferente:
\begin{itemize}
	\item No divide el problema en dos etapas.
	\item Comienza con $k$ clusters vacíos.
	\item En cada iteración siempre agrega un ítem.
	\item El ítem se agrega al cluster que maximice la función objetivo sin importar que el nuevo valor de la función objetivo sea menor al actual.
	\item El algoritmo va a generar soluciones en las que se prioriza completar los paquetes con la máxima cantidad de ítems en cada paquete.
\end{itemize}
}
...
\begin{itemize}
	\item Base de datos proporcionada por \textit{Data Driven Journey through Software Engineering Research}.
	...
	\item Catalogados por autores, tópicos relevantes, conferencia donde fue presentando el trabajo y afiliaciones de los autores.
	...
	\item A partir del \textit{topic profile} de los artículos, definimos los perfiles para los autores y afiliaciones.
\end{itemize}
}
...
\begin{itemize}
	...
	\item Consulta de autores con un perfil similar de distintas afiliaciones.
	\item Función de similitud que compara el perfil de cada autor.
	\item Atributo de complementariedad definido como la afiliación del autor.
	%\item Una unidad de costo para cada artículo o autor y cinco de presupuesto $\beta$ (todos los paquetes contendrán como máximo cinco artículos o autores).
	\item Una unidad de costo para cada autor y cinco de presupuesto $\beta$ (todos los paquetes contendrán como máximo cinco autores).
	\item $k = 10$ paquetes retornados en cada consulta (para que un humando pueda valorizar el resultado propuesto fácilmente).
	\item Pruebas realizadas con la siguiente configuración: Desktop Intel(R) Core(TM) $i5-4570$T CPU @ $2.9GHz$, $5.7Gb$ Ram, base de datos $5.5.46$-MariaDB-1ubuntu$0.14.04.2$. El lenguaje utilizado fue $C++$.
\end{itemize}
\normalsize
}
...
% 	\item Algoritmo $BOBO$: no obtienen soluciones de la calidad de los algoritmos $C-HAC$.
...
% 		\item En soluciones de mejor calidad (algoritmo $C-HAC$) para los valores de $\gamma$ la solución obtenida resultó ser la mejor opción.
% 	\end{itemize}
% \end{itemize}
% }

\frame{\frametitle{$BOBO\ \gamma = 0,1$}
\makebox[\textwidth]{\includegraphics[width=0.73\paperwidth]{imagenes/gamma-01-alg-3.png}}
}

\frame{\frametitle{$BOBP+T\ \gamma = 0,1$}
\makebox[\textwidth]{\includegraphics[width=0.73\paperwidth]{imagenes/gamma-01-alg-4.png}}
}

\frame{\frametitle{$BOBO\ \gamma = 0,9$}
\makebox[\textwidth]{\includegraphics[width=0.73\paperwidth]{imagenes/gamma-09-alg-3.png}}
}

\frame{\frametitle{$BOBP+T\ \gamma = 0,9$}
\makebox[\textwidth]{\includegraphics[width=0.73\paperwidth]{imagenes/gamma-09-alg-4.png}}
}
