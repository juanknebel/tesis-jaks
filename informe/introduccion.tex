...
\end{small}%
\vspace{.5cm}}
Los motores de búsqueda son sistemas informáticos que buscan elementos almacenados en distintos servidores. Se pueden encontrar tres clases de buscadores: los jerárquicos, por directorios o metabuscadores.

Los jerárquicos cuentan con ``arañas'' (spiders) que recorren las páginas recopilando la información contenida en ellas. También cuentan con programas específicos encargados de clasificar según su relevancia todo el contenido encontrado. Este proceso es muy dinámico y se encuentra completamente automatizado, necesitando mucho poder de procesamiento y algoritmos más complejos.

Los buscadores por directorio, a diferencia de los jerárquicos, están basados en algoritmos mucho más simples. Solo buscan dentro de una base de datos que contiene poca información acerca de las páginas, solo la ingresada al inicio de la registración de la página. El contenido almacenado es normalmente revisado y catalogado por operadores humanos, ofreciendo resultados dentro de categorias mejor definidas.

La última clasificación son los metabuscadores, los cuales utilizan los buscadores de las dos categorias antes mencionadas y solo aplican un tipo de orden definido por el propio metabuscador. Se los podría llamar ``buscador de buscadores'' ya que permite realizar busquedas en varios el mismo tiempo.

Una cuarta clasificación que se suele nombrar son los buscadores verticales, que simplemente son buscadores especializados en alguna temática particular. De esa manera les permite analizar la información con una mayor profundidad y poseer resutlados más actualizados.

Sin importar el tipo de buscador que estemos usando todos tienen el mismo proposito: \underline{brindarle al usuario resultados relevantes a sus búsquedas}. Normalmente los resultados son presentados en forma de una lista ordenada.
...
Los primeros motores de búsqueda solo admitían una palabra o frase que el usuario ingresaba, entonces la tarea de los llamados ``buscadores'' consistía en encontrar una coincidencia exacta dentro de una colección de documentos. A medida que la cantidad de información comenzó a incrementarse, las estrategias iniciales dejaron de ser útiles. El tiempo de respuesta se deterioró considerablemente y la calidad de los resultados no era la esperada por los usuarios. Por lo tanto, las formas de buscar fueron evolucionando y adaptándose a las necesidades del usuario con el objetivo final de entregar resultados completos y relevantes.
