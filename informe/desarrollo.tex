...
\end{itemize}		  
El parámetro $\gamma$ sirve para que el usuario pueda definir el balance entre que la solución contenga paquetes más cohesivos (intra) o la diversidad de los paquetes (inter) de una solución. Si El usuario prioriza una solución con paquetes cohesivos el valor de $\gamma$ será cercano a uno mientras que si lo que prioriza es una solución con paquetes diversos el valor estará cercano a cero.

En \cite{compositeRetrival} se demuestra que la complejidad de devolver $k$ paquetes de items complementarios con un presupuesto es NP-Completo, lo que implica que por el momento no se puede encontrar una solución exacta en tiempo polinomial. En base a esto, este trabajo se enfoca en encontrar soluciones suficientemente buenas para el problema en tiempo polimonial por lo que se implementaron los algoritmos Produce-and-Choose y el goloso.
