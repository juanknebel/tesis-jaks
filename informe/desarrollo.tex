\section{Modelo}
Dado el conjunto de items I la función de similitud s: IxI->R[0;1]. El problema se representa como un grafo 
con peso en las aristas donde los vertices son los items y el peso de las aristas es la similitud entres estos. 
\section{Problema}
...
Las diferencias con los problemas tipicos de clusterización son (1) que la cantidad de items en un bundle esta limitada por 
el presupuesto y (2) que el bundle no puede existir más de un item con el mismo atributo de complementaridad.
de encontar el subgrafo con k nodos 
\section{Función de similitud}
La similitud entre los papers esta dado por la comparación del perfil de cada uno. El perfil del paper se obtiene del topic profile, para hacer
la comparación se represento el topic profile cómo un vector de $N$ dimensiones, donde $N$ es la cantidad de tópicos, normalizados
ya que cada compenente representa un porcentaje del tema al que pertenece.\\
Dados estos vectores calcular la similitud entre los papers es la distancia angular entre ellos. Los vectores que formen un ángulo cercano a $0$\textdegree 
representan una similitud alta entre los perfiles, mientras que un ángulo cercano a $90$\textdegree dos perfiles ortogonales o diferentes.\\
