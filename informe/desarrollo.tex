...
\section{Modelo}
Dado el conjunto de objetos $I$ y una función de similitud $s: I \times I \rightarrow [0;1]$, cada objeto que contiene un conjunto de atributos queda unívocamente identificado. La entrada puede pensarse como un grafo completo con peso en las aristas $G=(I,E,s)$ donde el peso de la arista $(u,v)$ es $s(u,v)$. Se define también la \textit{función de distancia} $d(u,v) = 1 - s(u,v)$ que también toma valores en el intervalo $[0;1]$, un función $f: I \rightarrow (0;+\infty]$ que indica el costo de los objetos y un valor $\beta \in \Re+$ que servirá como cota máxima para formar un bundle.\\
El problema consiste en devolver un conjunto de bundles $S = \left\{s_1, \ldots, s_k\right\}$ donde el elemento $S_i \in 2^{I}$ es un conjunto de objetos que satisface las reglas de \textit{complementaridad} que no permite que existen dos objetos con igual atributo en el mismo bundle y de \textit{presupuesto} para que la suma de los costos de los objetos no exceda el presupuesto elegido.\\
Formalmente podemos definir \textit{Composite Retrieval} como:\\
Dado el conjunto de objetos $I=\left\{i_1,\ldots, i_n\right\}$, una función de similitud $s(u,v)$, una de costo $f$, un presupuesto $\beta$ (como se definió en previamente), un atributo complementario $\alpha$, un valor $\gamma \in (0:1)$ y un entero $k$ que indica la cantidad máxima de bundles que debe tener la solución final. Se desea hallar el conjunto válido de bundles $S = \left\{s_1, \ldots, s_k\right\}$ que maximiza la función:
...
En \cite{compositeRetrival} se demuestra que la complejidad de devolver $k$ bundles de items complementarios con un presupuesto es NP-Completo, lo que implica que por el momento no se puede encontrar una solución exacta en tiempo polinomial. En base a esto este trabajo se enfoca en encontrar soluciones suficientemente buenas para el problema en tiempos polinomiales se implementaron dos algoritmos: Produce-and-Choose y algoritmo goloso y se comparan.
...
En ~\ref{alg:PAC} se obseva los tres grandes pasos a realizar en la ejecución del algoritmo. En primer lugar se genera el conjunto \textit{cand} que contiene bundles válidos y solo algunos de ellos formarán parte de la solución final. Como segundo paso se genera el grafo \textit{G} no dirigido y completo en el que cada bundle es un vértice y las aristas con peso que representan la distancia o disimilitud entre los vértices que la forman. Por último se seleccionan los mejores \textit{k} bundles del grafo y se devuelven como la solución.
...
\end{figure}
En este trabajo la definición para la clusterización es que cada cluster contenga la máxima cantidad de items sin excederse del presupuesto. La agrupación se realiza por la similitud de los objetos, que se obtiene a través de la función de similitud, para que los items del cluster sean lo más parecido posible y así obtener bundles coehsivos.\\
