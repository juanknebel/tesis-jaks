...
La desición en esta tesis es la de incluir el valor \texttt{inter} en la selección del par de {\em clusters} que se unen, en pos de lograr un mejor criterio de unión. Como consecuencia, el conjunto de paquetes generados es más disperso. Por este motivo se definió la función \textit{Intra-Inter} en reemplazo de la función \textit{Score}. 
...
Retomando las fórmulas presentadas anteriormente, para evitar realizar el cálculo desde cero de $A$ y $E$ para todo par de {\em clusters} en todas las iteraciones de la etapa \emph{Produce} del algoritmo, se  mantienen estructuras con esos valores, cuya actualización se realiza en tiempo constante a partir de las siguientes relaciones entre la iteración $r$ y $r-1$:
...
Para mejorar la complejidad del algoritmo de \texttt{C-HAC} e incluir la información inter-paquete, aquí se propone el algoritmo \texttt{Intra-Inter C-HAC}, el cual tiene una complejidad $\mathcal{O}(N^{2}\lg n)$. 
