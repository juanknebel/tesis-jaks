En este capítulo se presentan nuevas propuestas para resolver el problema de \texttt{\\Recuperación de Ítems Empaquetados}. El foco principal de las mismas es la obtención de mejores soluciones que puedan ser generadas a partir de un dominio del problema que contiene gran cantidad de elementos disponibles respecto de las estrategias presentadas en \cite{journals/tkde/Amer-YahiaBCFMZ14}. La comparación entre soluciones es realizada en función del valor de la Función Objetivo presentada en la \autoref{des:eq-fnObj}.
...
Analizando la complejidad del\textbf{~\autoref{alg:C-HAC}} el principal ciclo se ejecuta $N - c$ veces, donde $N$ es la cantidad de ítems. En cada paso de la iteración principal se unen los dos conjuntos con mayor similitud siempre y cuando el conjunto resultante de esa unión sea válido.
...
Entonces, para \textit{Intra-Inter} dados dos {\em clusters} $C_i$ y $C_j$ se define $A(C_i,C_j)$ para representa la similitud entre $C_i$ y $C_j$ y $E(C_i,C_j)$ la inversa de la distancia.
		...
		\ForEach{$s_i\in S$}{
			\ForEach{$s_j\in S$}{
				\eIf{$validMerge(s_i,s_j,\alpha,f,\beta)$}{
					$C[i][j] \leftarrow Score(s_i \cup s_j)$\;
				}
				{
					$C[i][j] \leftarrow -1$\;
				}
			}
			$P[i] \leftarrow $ priority queue for $C[i]$ sorted \;
			$P[i].Delete(C[i][i])$ \tcc*[f]{se elimina así mismo de la pila}\;
		}
		\For{$n \leftarrow 1$ to $I.length$}{
			$k_1 \leftarrow \argmax_{k}{P[k].max()}$\;
			$k_2 \leftarrow \max P[k].max()$\;
			
			\If{$validMerge(k_1,k_2,\alpha,f,\beta)$}{
				$S \leftarrow S \cup \left\{k_1 \cup k_2 \right\} \setminus \left\{k_1\right\} \setminus \left\{k_2\right\} $\;
				\ForEach{$s_i\in S$}{
					$P[i].Delete(C[i][k_1])$\;
					$P[i].Delete(C[i][k_2])$\;
					\eIf{$validMerge(s_i,\left\{k_1 \cup k_2 \right\},\alpha,f,\beta)$}{
						$C[i][k_1] \leftarrow Inter-Intra(s_i,k_1 \cup k_2,\gamma)$\;
						$C[k_1][i] \leftarrow Inter-Intra(s_i,k_1 \cup k_2,\gamma)$\;
					}
					{
						$C[i][k_1] \leftarrow -1$\;
						$C[k_1][i] \leftarrow -1$\;
...
%Si se considerase en la etapa de producción tres paquetes $A$, $B$ y $C$ con los siguientes valores \texttt{Intra}: $\omega(A) = 5$, $\omega(B) = 5$ y $\omega(C) = 5$ y el valor \texttt{Inter} entre los paquetes: $\psi(A,B) = 0$, $\psi(A,C) = 1$ y $\psi(B,C) = 0$. Bajo ese escenario la solución que contenga dos paquetes con $\gamma=0,1$ se tiene los siguientes valores para cada par de paquetes:
			...
			\ForEach{$elem \in I$}{
				\ForEach{$bundle \in cand$}{
					\If{$validMerge(bundle,\{elem\},\alpha,f,\beta)$}{
						$score \leftarrow Score((Cand \setminus \left\{bundle\right\}) \cup \left\{bundle \cup \left\{elem\right\}\right\})$\;
						\If{$score > bestScore$}{
