...
void ProblemInstanceFromDataBase::init(Dao *dao, String tableCosts, String tableCompat, String tableCover, String costField, String compatField, String coverField, String primaryField, String secondaryField, String primaryDescription) {
	this->dao_ = dao;
	...
	this->secondaryField_ = secondaryField;//ArticleId2
...
ProblemInstanceFromDataBase::ProblemInstanceFromDataBase(Dao *dao, String tableCosts, String tableCompat, String tableCover, String costField, String compatField, String coverField, String primaryField, String secondaryField, String primaryDescription, Double budget) : ProblemInstance(budget) {
	this->init(dao, tableCosts, tableCompat, tableCover, costField, compatField, coverField, primaryField, secondaryField, primaryDescription);
}
...
IntSet& ProblemInstanceFromDataBase::getIds() {
	if (this->ids_->empty()) {
		if (this->dao_->executeSelectAllFromProject(this->tableCosts_, this->primaryField_)) {
			const char** result;
...
Double ProblemInstanceFromDataBase::getCompat(int id1, int id2) {
	Double compat = -1.0;
	const char *fields [] =  {this->primaryField_.c_str(), this->secondaryField_.c_str()};
	const char *values [] = {convertToString(id1).c_str(), convertToString(id2).c_str()};
	if (this->dao_->executeSelectProjectFromWithAndConditions(this->tableCosts_, this->costField_,fields, values, 2)) {
