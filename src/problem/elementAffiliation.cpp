...
void ElementAffiliation::completeDataForQtModel(const std::vector<SnowFlake> &solution, QStandardItemModel *model) const
{
    std::map<std::string, std::string> *node2name = this->node2name_.get();
    int row = 0;
    for (auto aFlake : solution) {
    ...
    QStringList labelList = {"Bundle", "Afiliación"};
