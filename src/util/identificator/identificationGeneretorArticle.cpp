...
void IdentificationGeneretorArticle::fillTheMapping(std::map<std::string, std::string> *node2name) {
    if (this->dao_->executeCustomConsultativeQuery(this->query_)) {
        while (this->dao_->fetch()) {
            ...
            if (it == node2name->end()) {
                (*(node2name))[this->dao_->getField(1)] = this->dao_->getField(1) + this->separator_ + this->dao_->getField(2) + this->separator_ + this->dao_->getField(3) + this->separator_  + this->dao_->getField(4);
            }
            else {
